...
namespace BBDown
{
    internal class BBDownDownloadUtil
    {
        ...
        private static async Task RangeDownloadToTmpAsync(
            int id,
            string url,
            string tmpName,
            long fromPosition,
        ...
        )
        {
            ...
            using var response = (
                await AppHttpClient.SendAsync(httpRequestMessage, HttpCompletionOption.ResponseHeadersRead)
            ...
            var totalBytes =
                downloadedBytes + (response.Content.Headers.ContentLength ?? long.MaxValue - downloadedBytes);
            ...
            if (
                ...
                && (response.Content.Headers.ContentLength != new FileInfo(tmpName).Length)
            )
        ...
        public static async Task DownloadFile(string url, string path, DownloadConfig config)
        {
            ...
            if (!string.IsNullOrEmpty(desDir) && !Directory.Exists(desDir))
                ...
                async (clip, _) =>
                {
                    ...
                    try
                    {
                        await RangeDownloadToTmpAsync(
                            ...
                            tmp,
                            clip.from,
                            clip.to == -1 ? null : clip.to,
                            (index, downloaded, _) =>
                            {
                                clipProgress[index] = downloaded;
                                progress.Report(
                                    (double)clipProgress.Values.Sum() / fileSize,
