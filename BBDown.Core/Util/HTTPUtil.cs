...
namespace BBDown.Core.Util
{
    public class HTTPUtil
    {
        ...
        private static string GetRandomUserAgent()
        {
            string[] browsers =
            {
                $"AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{RandomVersion(80, 110)} Safari/537.36",
                $"Gecko/20100101 Firefox/{RandomVersion(80, 110)}",
            };
        ...
        public static async Task<string> GetWebSourceAsync(string url)
        {
            ...
            webRequest.Headers.TryAddWithoutValidation(
                "Cookie",
                (url.Contains("/ep") || url.Contains("/ss")) ? Config.COOKIE + ";CURRENT_FNVAL=4048;" : Config.COOKIE
            );
            ...
            var webResponse = (
                await AppHttpClient.SendAsync(webRequest, HttpCompletionOption.ResponseHeadersRead)
        ...
        public static async Task<byte[]> GetPostResponseAsync(
            string Url,
            byte[] postData,
