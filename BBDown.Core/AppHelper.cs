...
static class AppHelper
{
    ...
    /// <returns></returns>
    public static async Task<string> DoReqAsync(
        string aid,
        string cid,
        string epId,
        string qn,
    ...
    private static string ConvertToDashJson(object data)
    {
        ...
        {
            audios.AddRange(
                resp.VideoInfo.DashAudio.Select(item => new AudioInfoWithCodecName(
                    item.Id,
                    item.BaseUrl,
        ...
        {
            clips.AddRange(resp.Business.ClipInfo.Select(clip => new DashClip(clip.Start, clip.End, clip.ToastText)));
        }

        var backgroundAudios = new List<object>();
        var roles = new List<object>();
        if (
            resp.PlayExtInfo != null
            && resp.PlayExtInfo.PlayDubbingInfo != null
            && resp.PlayExtInfo.PlayDubbingInfo.BackgroundAudio != null
        )
        {
            var dubInfo = resp.PlayExtInfo.PlayDubbingInfo;

            backgroundAudios.AddRange(
                dubInfo.BackgroundAudio.Audio.Select(item => new AudioInfoWithCodecName(
                    item.Id,
                    item.BaseUrl,
                    item.BackupUrl.ToList(),
                    item.Bandwidth,
        ...
        var json = new DashJson(
            ...
            "0",
            1,
            new DashData(resp.VideoInfo.Timelength, new DashInfo(videos, audios), clips),
            new DubbingInfo(backgroundAudios, roles)
        );
    ...
    private static Dictionary<string, string> GetHeader(string appkey)
    {
        return new Dictionary<string, string>()
        {
            ...
            ["user-agent"] =
                $"Dalvik/{dalvikVer} (Linux; U; Android {osVer}; {brand} {model}) {appVer} os/android model/{brand} mobi_app/android build/{build} channel/{channel} innerVer/{build} osVer/{osVer} network/2 grpc-java-cronet/{cronet}",
            ["te"] = "trailers",
    ...
    private static string GenerateLocaleBin()
    {
        var obj = new Locale
        {
            CLocale = new Locale.Types.LocaleIds { Language = language, Region = region },
        };
    ...
    {
        var obj = new Network { Type = networkType, Oid = networkOid };
        return Convert.ToBase64String(obj.ToByteArray());
    }
...
internal class DubbingInfo
{
    ...
    public override bool Equals(object? obj) =>
        obj is DubbingInfo other && BackgroundAudio == other.BackgroundAudio && RoleAudioList == other.RoleAudioList;
...
internal class DashClip
{
    ...
    public override bool Equals(object? obj) =>
        obj is DashClip other && Start == other.Start && End == other.End && ToastText == other.ToastText;
...
internal class AudioInfoWithCodecName
{
    ...
    public override bool Equals(object? obj) =>
        obj is AudioInfoWithCodecName other
        && Id == other.Id
        && BaseUrl == other.BaseUrl
        && BackupUrl.SequenceEqual(other.BackupUrl)
...
internal class AudioInfoWitCodecId
{
    ...
    public override bool Equals(object? obj) =>
        obj is AudioInfoWitCodecId other
        && Id == other.Id
        && BaseUrl == other.BaseUrl
        && Bandwidth == other.Bandwidth
...
internal class DashInfo
{
    ...
    public override bool Equals(object? obj) =>
        obj is DashInfo other
        && EqualityComparer<List<object>>.Default.Equals(Video, other.Video)
        && EqualityComparer<List<object>>.Default.Equals(Audio, other.Audio);
...
internal class DashData
{
    ...
    public override bool Equals(object? obj) =>
        obj is DashData other
        && TimeLength == other.TimeLength
        && EqualityComparer<DashInfo>.Default.Equals(Dash, other.Dash)
        && EqualityComparer<List<object>>.Default.Equals(ClipList, other.ClipList);
...
internal class DashJson
{
    ...
    public override bool Equals(object? obj) =>
        obj is DashJson other
        && Code == other.Code
        && Message == other.Message
        && Ttl == other.Ttl
        && EqualityComparer<DashData>.Default.Equals(Data, other.Data);
