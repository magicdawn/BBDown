...
namespace BBDown.Core
{
    public partial class Parser
    {
        public static string WbiSign(string api)
        {
            return $"{api}&w_rid="
                + string.Concat(
                    MD5.HashData(Encoding.UTF8.GetBytes(api + Config.WBI)).Select(i => i.ToString("x2")).ToArray()
                );
        }

        private static async Task<string> GetPlayJsonAsync(
            string encoding,
            string aidOri,
            string aid,
            string cid,
        ...
        {
            LogDebug(
                "aid={0},cid={1},epId={2},tvApi={3},IntlApi={4},appApi={5},qn={6}",
                aid,
                cid,
                epId,
                tvApi,
            ...
            if (intl)
                return await GetPlayJsonAsync(aid, cid, epId, qn);
            ...
            if (appApi)
                return await AppHelper.DoReqAsync(aid, cid, epId, qn, bangumi, encoding, Config.TOKEN);

            string prefix = tvApi
                ? bangumi
                    ? "api.snm0516.aisee.tv/pgc/player/api/playurltv"
                    : "api.snm0516.aisee.tv/x/tv/playurl"
                : bangumi
                    ? $"{Config.HOST}/pgc/player/web/v2/playurl"
            ...
            if (tvApi)
            {
                ...
                if (Config.TOKEN != "")
                    apiBuilder.Append($"access_key={Config.TOKEN}&");
            ...
            else
            {
                ...
                StringBuilder apiBuilder = new();
                apiBuilder.Append(
                    $"support_multi_audio=true&from_client=BROWSER&avid={aid}&cid={cid}&fnval=4048&fnver=0&fourk=1"
                );
                if (Config.AREA != "")
                    apiBuilder.Append($"&access_key={Config.TOKEN}&area={Config.AREA}");
        ...
        )
        {
            ...
            if (Config.TOKEN != "")
                paramBuilder.Append($"access_key={Config.TOKEN}&");
            paramBuilder.Append($"aid={aid}");
            if (isBiliPlus)
                paramBuilder.Append($"&appkey=7d089525d3611b1c&area={(Config.AREA == "" ? "th" : Config.AREA)}");
        ...
        )
        {
            ...
            //调用解析
            parsedResult.WebJsonString = await GetPlayJsonAsync(
                encoding,
                aidOri,
                aid,
                cid,
            ...
            {
                int pDur =
                    data.GetProperty("data").GetProperty("video_info").GetProperty("timelength").GetInt32() / 1000;
                var audio = data.GetProperty("data")
                    .GetProperty("video_info")
                    .GetProperty("dash_audio")
                    .EnumerateArray()
                    .ToList();
                foreach (
                    var stream in data.GetProperty("data")
                        .GetProperty("video_info")
                        .GetProperty("stream_list")
                        .EnumerateArray()
                )
                {
                    if (stream.TryGetProperty("dash_video", out JsonElement dashVideo))
                    {
                        if (dashVideo.GetProperty("base_url").ToString() != "")
                        {
                            ...
                            urlList.AddRange(
                                dashVideo.GetProperty("backup_url").EnumerateArray().Select(i => i.ToString())
                            );
                            Video v =
                                new()
                                {
            ...
            var root =
                nodeName == null ? data
                : nodeName == "video_info" ? data.GetProperty("result").GetProperty(nodeName)
                : data.GetProperty(nodeName);
            ...
            if (parsedResult.WebJsonString.Contains("\"dash\":{")) //dash
            {
                ...
                try
                {
                    pDur = root.GetProperty("dash").GetProperty("duration").GetInt32();
                }
                catch { }
                try
                {
                    pDur = root.GetProperty("timelength").GetInt32() / 1000;
                }
                catch { }
                ...
                try
                {
                    video = root.GetProperty("dash").GetProperty("video").EnumerateArray().ToList();
                }
                catch { }
                try
                {
                    audio = root.GetProperty("dash").GetProperty("audio").EnumerateArray().ToList();
                }
                catch { }
                ...
                if (appApi && bangumi)
                {
                    try
                    {
                        backgroundAudio = data.GetProperty("dubbing_info")
                            .GetProperty("background_audio")
                            .EnumerateArray()
                            .ToList();
                    }
                    catch { }
                    try
                    {
                        roleAudio = data.GetProperty("dubbing_info")
                            .GetProperty("role_audio_list")
                            .EnumerateArray()
                            .ToList();
                    }
                    catch { }
                ...
                if (video != null)
                {
                    foreach (var node in video)
                    {
                        var urlList = new List<string>() { node.GetProperty("base_url").ToString() };
                        if (
                            node.TryGetProperty("backup_url", out JsonElement element)
                ...
                if (backgroundAudio != null && roleAudio != null)
                {
                    ...
                    foreach (var role in roleAudio)
                    {
                        ...
                        parsedResult.RoleAudioList.Add(
            ...
            else if (parsedResult.WebJsonString.Contains("\"durl\":[")) //flv
            {
                ...
                {
                    parsedResult.Dfns.AddRange(
                        qnExtras.EnumerateArray().Select(node => node.GetProperty("qn").ToString())
                    );
                }
            ...
            if (bangumi)
            {
                ...
                {
                    parsedResult.ExtraPoints.AddRange(
                        clipList
                            .EnumerateArray()
                            .Select(clip => new ViewPoint()
                            {
        ...
        {
            string toEncode =
                parms + (isBiliPlus ? "acd495b248ec528c2eed1e862d393126" : "59b43e04ad6965f34319062b478f83dd");
            return string.Concat(
                MD5.HashData(Encoding.UTF8.GetBytes(toEncode)).Select(i => i.ToString("x2")).ToArray()
            );
        }
